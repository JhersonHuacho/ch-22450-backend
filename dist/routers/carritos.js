"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
// import ContenedorCarrito from '../utilitario/Carrito';
var index_1 = require("../daos/index");
var router = (0, express_1.Router)();
// const objCarrito = new ContenedorCarrito('db');
// Router de carritos
router.get('/', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, _b;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                // res.json((await carritosApi.getAll()).map((c: any) => c.id));
                _b = (_a = res).json;
                return [4 /*yield*/, index_1.carritosDao.getAll()];
            case 1:
                // res.json((await carritosApi.getAll()).map((c: any) => c.id));
                _b.apply(_a, [_c.sent()]);
                return [2 /*return*/];
        }
    });
}); });
router.post('/', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, _b;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                _b = (_a = res).json;
                return [4 /*yield*/, index_1.carritosDao.save(req.body)];
            case 1:
                _b.apply(_a, [_c.sent()]);
                return [2 /*return*/];
        }
    });
}); });
router.delete('/:id', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, _b;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                _b = (_a = res).json;
                return [4 /*yield*/, index_1.carritosDao.deleteById(req.params.id)];
            case 1:
                _b.apply(_a, [_c.sent()]);
                return [2 /*return*/];
        }
    });
}); });
// Router de productos en carrito
router.get('/:id/productos', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var carrito;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, index_1.carritosDao.getById(req.params.id)];
            case 1:
                carrito = _a.sent();
                console.log('get => /:id/productos => carrito', carrito);
                console.log('get => /:id/productos => carrito', JSON.stringify(carrito.productos));
                // res.json(JSON.stringify(carrito.productos));
                res.json(carrito[0].productos);
                return [2 /*return*/];
        }
    });
}); });
router.post('/:id/productos', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var carrito, producto;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, index_1.carritosDao.getById(req.params.id)];
            case 1:
                carrito = _a.sent();
                return [4 /*yield*/, index_1.productosDao.getById(req.body.id)];
            case 2:
                producto = _a.sent();
                carrito.productos.push(producto);
                return [4 /*yield*/, index_1.carritosDao.update(carrito)];
            case 3:
                _a.sent();
                res.end();
                return [2 /*return*/];
        }
    });
}); });
router.delete('/:id/productos/:id_prod', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var carrito, index;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, index_1.carritosDao.getById(req.params.id)];
            case 1:
                carrito = _a.sent();
                index = carrito.productos.findIndex(function (p) { return p.id == req.params.id_prod; });
                if (!(index != -1)) return [3 /*break*/, 3];
                carrito.productos.splice(index, 1);
                return [4 /*yield*/, index_1.carritosDao.update(carrito)];
            case 2:
                _a.sent();
                _a.label = 3;
            case 3:
                res.end();
                return [2 /*return*/];
        }
    });
}); });
exports.default = router;
//# sourceMappingURL=carritos.js.map